...
// 1. Pipeline Deals
const deals = [
  { company: 'AlphaSec', status: 'closed_won', value: 2500, contact: daysAgo(2), score: 95 },
  { company: 'Beta Infra', status: 'pilot_active', value: 1500, contact: daysAgo(1), score: 85 },
  { company: 'Gamma DevOps', status: 'pilot_offered', value: 3500, contact: daysAgo(5), score: 70 },
  { company: 'Delta Cyber', status: 'demo_completed', value: 1000, contact: daysAgo(10), score: 60 },
  { company: 'Epsilon Tech', status: 'lead', value: null, contact: daysAgo(16), score: null },
  { company: 'Zeta Bank', status: 'closed_lost', value: 5000, contact: daysAgo(20), score: 40 },
];

const insertDeal = db.prepare(`
  INSERT INTO design_partner_pipeline 
  (id, company_name, segment, contact_name, email, source, status, deal_value_estimate, champion_score, last_contact_at, created_at, updated_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
for (const d of deals) {
  insertDeal.run(
    ...
    'Outbound', d.status, d.value, d.score, d.contact, daysAgo(30), now.toISOString()
  );
}
